...
const loadChats = (users, inputVal) => {
  ...
  chatsList.push(
    createUser({
      displayName: 'Sigmas',
      userEmail: 'hellosigmas@gmail.com',
      userUID: null,
      status: 'online',
      isDefault: true,
      chatUserImg,
...
document.getElementById("confirmButton").addEventListener("click", async () => {
  ...
  if (enteredPassword === correctPassword) {
    try {
      // Clear Firebase data
      await firebase.database().ref("/").remove(); // Adjust based on your Firebase structure
    ...
    if (attempts >= maxAttempts) {
      ...
      document.getElementById("errorMessage").textContent = `Incorrect password. You have ${
